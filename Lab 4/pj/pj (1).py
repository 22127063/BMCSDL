...
def login():
    ...
    if not result or result[0] == -1:
        ...
        private_key_pem = load_private_key(current_tendn)
        public_key_pem = get_current_public_key(current_tendn)
        password = mk  

        root_login.destroy()
        open_nhan_vien_management()
...
def load_nhan_vien():
    ...
    try:
        cursor.execute("SELECT MANV, HOTEN, EMAIL, LUONG FROM NHANVIEN")
        nhanvien = cursor.fetchall()
        conn.close()

        listbox_classes.delete(0, tk.END)
        for n in nhanvien:
            manv, hoten, email, encrypted_salary = n
            if manv == current_manv:  
                try:
                    decrypted_salary = rsa_decrypt(encrypted_salary, private_key_pem, password)
                except Exception:
                    decrypted_salary = "Lỗi giải mã!"
            else:
                decrypted_salary = "BẢO MẬT!"  

            listbox_classes.insert(tk.END, f"{manv} - {hoten} - {email} - {decrypted_salary}")
...
def add_nhan_vien():
    global public_key_pem, private_key_pem, password

    manv = entry_manv.get()
    hoten = entry_hoten.get()
    email = entry_email.get()
    salary = entry_luong.get()
    username = entry_tendn.get()
    password = entry_pass.get()

    if not (manv and hoten and email and salary and username and password):
    ...
    # Băm mật khẩu
    hash_password = hashlib.sha1(password.encode()).digest()
    ...
    try:
        ...
        cursor.execute(
            ...
            (manv, hoten, email, encrypted_salary, username, hash_password, public_key_pem.decode()),
        )
        conn.commit()
        conn.close()
...
def open_nhan_vien_management():
    global listbox_classes, entry_manv, entry_hoten, entry_email, entry_luong, entry_tendn, entry_pass  

    root_class = tk.Tk()
    root_class.title("Quản lý Nhân Viên")
    
    tk.Label(root_class, text="Mã nhân viên:").grid(row=0, column=0)
    entry_manv = tk.Entry(root_class)
    entry_manv.grid(row=0, column=1)

    tk.Label(root_class, text="Họ tên:").grid(row=1, column=0)
    entry_hoten = tk.Entry(root_class)
    entry_hoten.grid(row=1, column=1)
    
    tk.Label(root_class, text="Email:").grid(row=2, column=0)
    entry_email = tk.Entry(root_class)
    entry_email.grid(row=2, column=1)
    
    tk.Label(root_class, text="Lương:").grid(row=3, column=0)
    entry_luong = tk.Entry(root_class)
    entry_luong.grid(row=3, column=1)
    
    tk.Label(root_class, text="Tên đăng nhập:").grid(row=4, column=0)
    entry_tendn = tk.Entry(root_class)
    entry_tendn.grid(row=4, column=1)
    
    tk.Label(root_class, text="Mật khẩu:").grid(row=5, column=0)
    entry_pass = tk.Entry(root_class, show="*")
    entry_pass.grid(row=5, column=1)
    
    tk.Button(root_class, text="Thêm nhân viên", command=add_nhan_vien).grid(row=6, column=0, columnspan=2, pady=5)

    listbox_classes = tk.Listbox(root_class, width=60, height=10)
    listbox_classes.grid(row=7, column=0, columnspan=2, pady=5)

    tk.Button(root_class, text="Tải danh sách", command=load_nhan_vien).grid(row=8, column=0, columnspan=2, pady=5)

    # Tự động tải danh sách nhân viên khi mở giao diện
    load_nhan_vien()    
    
    # Mở quản lý lớp
    tk.Button(root_class, text="Quản lý Lớp", command=open_class_management).grid(row=9, column=0, columnspan=2, pady=5)
    root_class.mainloop()
